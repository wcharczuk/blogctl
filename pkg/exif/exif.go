...
// IsTagNotPresentError returns if an error is a tag not present error.
...
// IsCriticalError returns if given the error returned by Decode, reports whether the
...
// Values returns the raw values for the exif data.
func (x *Exif) Values() map[string]interface{} {
	output := make(map[string]interface{})
	for field, value := range x.Fields {
		output[string(field)] = value.String()
	}
	return output
}
...
// TimeZone returns the timezone on the exif, only works for canon.
